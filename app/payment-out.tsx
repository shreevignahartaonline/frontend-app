...
export default function PaymentOutScreen() {
  ...
  // State management
  const [paymentsOut, setPaymentsOut] = useState<Payment[]>([]);
  ...
  const loadPaymentData = async () => {
    try {
      const paymentsData = await PaymentApiService.getPaymentOutPayments();
      setPaymentsOut(paymentsData);
  ...
  const handlePartySelect = async (party: Party) => {
    ...
    try {
      ...
      setPaymentForm(prev => ({
        ...
        totalAmount: netBalance < 0 ? Math.abs(netBalance).toString() : '0',
      }));
    } catch (error) {
  ...
  const handleCreatePayment = async () => {
    ...
    try {
      ...
      const missingFields = [];
      if (!paymentForm.partyName.trim()) missingFields.push('Supplier Name');
      ...
      generateAndSendPDF(newPayment, paidAmount).catch(error => {
  ...
  const generateAndSendPDF = async (payment: Payment, amount: number) => {
    try {
      const pdfUri = await BasePdfGenerator.generatePaymentOutPDF({
        id: payment.id,
      ...
      if (pdfUri && payment.phoneNumber) {
        const whatsappResult = await DocumentService.sendPaymentVoucherViaWhatsApp(
          payment.phoneNumber,
          pdfUri,
          payment.paymentNo,
          payment.partyName,
        ...
        if (whatsappResult.success) {
          console.log('Payment voucher sent via WhatsApp successfully:', whatsappResult.messageId);
        } else {
          console.warn('Failed to send payment voucher via WhatsApp:', whatsappResult.error);
        }
      }
    } catch (error) {
  ...
  const handleSharePDF = async (payment: Payment) => {
    try {
      ...
      const success = await BasePdfGenerator.generateAndSharePaymentVoucher(paymentVoucher);
      if (!success) {
              ...
              {showPartyDropdown && filteredParties.length > 0 && (
                <View style={styles.partySuggestions}>
                  <FlatList
                    ...
                    keyExtractor={(item) => `payment-out-party-${item.id}`}
                    scrollEnabled={false}
              ...
              {selectedParty && (() => {
                ...
                return (
                  <View style={[
                    ...
                    <Ionicons 
                      name={netBalance < 0 ? "arrow-down-circle" : netBalance > 0 ? "arrow-up-circle" : "checkmark-circle"} 
                      size={20} 
                      color={netBalance < 0 ? Colors.error : netBalance > 0 ? Colors.success : Colors.textSecondary} 
                    />
                    <Text style={[
        ...
        <View style={styles.listContainer}>
          ...
          {paymentsOut.length > 0 ? (
            <FlatList
              ...
              keyExtractor={(item) => `payment-out-list-${item.id}`}
              scrollEnabled={false}
              showsVerticalScrollIndicator={false}
            />
          ) : (
